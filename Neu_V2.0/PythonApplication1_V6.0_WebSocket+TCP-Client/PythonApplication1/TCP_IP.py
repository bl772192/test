...
class PyWSock:
    ...
    def handle_client (self, client, addr):
        from TCP_IP import TCP_Client
        self.handshake(client)
        try:
            while 1:            
                data = self.recv_data(client)
                print("received [%s]" % (data,))
                New_Client=TCP_Client(('localhost', 9877), data)
                New_Client.run()
    ...
    def run(self):
            ...
            try:  
                ...
                while amount_received < amount_expected:
                    scan_value = sock.recv(128)
                    amount_received += len(scan_value)
                    print >>sys.stderr, 'Received ID: "%s"' % scan_value
